...
const char* AttackTime::name = "AttackTime";
const char* AttackTime::category = "Envelope/SFX";
const char* AttackTime::description = DOC("This algorithm computes the attack time of a signal envelope. The attack time is defined as the time duration from when the sound becomes perceptually audible to when it reaches its maximum intensity. By default, the start of the attack is estimated as the point where the signal envelope reaches 20% of its maximum value in order to account for possible noise presence. Also by default, the end of the attack is estimated as as the point where the signal envelope has reached 90% of its maximum value, in order to account for the possibility that the max value occurres after the attack, as in trumpet sounds.\n\n"
"With this said, AttackTime's input is intended to be fed by the output of the Envelope algorithm. In streaming mode, the RealAccumulator algorithm should be connected between Envelope and AttackTime.\n\n"
...
void AttackTime::configure() {
  ...
  if (_startThreshold > _stopThreshold) {
    throw EssentiaException("AttackTime: stopAttackThreshold is not greater than startAttackThreshold");
  }
}

void AttackTime::compute() {

  const std::vector<Real>& signal = _signal.get();
  Real& attackTime = _attackTime.get();
  Real& attackStart = _attackStart.get();
  Real& attackStop = _attackStop.get();

  if (signal.empty()) {
    throw EssentiaException("AttackTime: attackTime not defined for empty input");
  }
